...
int32_t g_stepper_vel[4], g_servo_torque[3];
...
int main(int argc, char **argv)
{
  ...
  double stepper_pos[4], servo_pos[3];
  ...
  while (n.ok())
  {
    ...
    if ((t - t_prev).toSec() > 0.05)
    {
      ...
      for (int i = 0; i < 3; i++)
      {
        servo_pos[i] += d * 0.1 * g_servo_torque[i];
        sensors_msg.pos[i+4] = (int32_t)g_servo_torque[i];
      }
